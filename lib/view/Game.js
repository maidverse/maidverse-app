...
const InitMaidPopup_1 = __importDefault(require("../ui/maids/InitMaidPopup"));
const SocialPanel_1 = __importDefault(require("../ui/SocialPanel"));
const ResultPopup_1 = __importDefault(require("../ui/store/ResultPopup"));
const UserPanel_1 = __importDefault(require("../ui/UserPanel"));
class Game {
    constructor() {
        this.codeStore = new Store_1.default("codeStore");
        this.checkBuyDebouncer = new debouncer_1.default(100, (address) => this.checkBuy(address));
        this.repositeUI = () => {
    ...
    async checkBuy(address) {
        const code = this.codeStore.get("code");
        if (code !== undefined) {
            try {
                const result = await superagent_1.default.get(`https://${Config_1.default.backendHost}/checkbuy`).query({ address, code });
                if (result.body.length > 0) {
                    new ResultPopup_1.default(result.body);
                }
            }
            catch (error) {
                console.error(error);
